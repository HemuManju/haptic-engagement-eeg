...
engagement.model <- lmer(response ~ 1 + (1 | df$hand_type), data = df)
...
model2 <- lmer(response ~ control_type + (1 | subject),
               data = df_dominant)
summary(model2)
tab_model(model2)
```
...
df$control_type <- relevel(df$control_type, "no_force")
model3 <- lmer(response ~ control_type + (1 | subject),
               data = df_non_dominant)
summary(model3)
tab_model(model3)
```
